...
# Media mapping regex (optional, can be empty for default behavior)
...
class ZabbixAPI:
    ...
    def get_media_types(self):
        """Get available media types from Zabbix"""
        media_types = self.call("mediatype.get", {"output": ["mediatypeid", "name", "type"]})
        return {mt["name"]: mt for mt in media_types}
...
def detect_media_config(group_name, available_media_names):
    """
    ...
    media_name = MEDIA_TYPE_EMAIL if matched else DEFAULT_MEDIA_TYPE
    
    if media_name and media_name in available_media_names:
        media_configs.append({
            "mediatype": {"name": media_name},
            "sendto": "",  # Will be populated from LDAP attribute per-user
            "active": 0,    # 0 = enabled, 1 = disabled
            "severity": 63, # All severities: 1+2+4+8+16+32
            "period": "1-7,00:00-24:00"  # Daily, 24/7
        })
...
def _normalize_mapping(m):
    """Normalize mapping dict for comparison (ignore dynamic fields)"""
    return {
        "name": m.get("name"),
        "roleid": m.get("roleid"),
        "user_groups": sorted(
...
def _normalize_media(m):
    ...
    return {
        "name": m.get("name"),
        "mediatype": m.get("mediatype", {}).get("name"),
        "sendto": m.get("sendto", ""),
        "active": m.get("active"),
...
def media_mappings_equal(old, new):
    """Compare provision_media lists ignoring order and auto-generated IDs"""
    if len(old) != len(new):
        return False
...
def main():
    ...
    available_media_names = {mt["name"] for mt in media_types.values()}
    logging.info(f"Available media types: {available_media_names}")
    ...
    for ldap_group in ldap_groups:
        ...
        # Media type mapping
        media_configs = detect_media_config(ldap_group, available_media_names)
        for mc in media_configs:
            provision_media_entry = {
                "name": ldap_group,
                "mediatype": mc["mediatype"],
                "sendto": "",  # Populated from LDAP attribute per-user
                "active": mc["active"],
        ...
        media_info = f" + media: {[m['mediatype']['name'] for m in media_configs]}" if media_configs else ""
    ...
    # Merge existing + new (preserve unrelated entries)
